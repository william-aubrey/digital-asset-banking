...
def transform(p):
    ...
    if rand_val < 0.01:
        # Transformation 1: Creates the "stem" (50% probability)
        x_new = 0.0
        y_new = 0.16 * y
    elif rand_val < 0.86:
        # Transformation 2: Creates the successive, smaller leaflets (41% probability)
        x_new = 0.85 * x + 0.04 * y
        y_new = -0.04 * x + 0.85 * y + 1.6
    elif rand_val < 0.93:
        # Transformation 3: Creates the largest left-hand leaflet (9% probability)
        x_new = 0.2 * x - 0.26 * y
        y_new = 0.23 * x + 0.22 * y + 1.6
    else:
        # Transformation 4: Creates the largest right-hand leaflet (9% probability)
        x_new = -0.15 * x + 0.28 * y
