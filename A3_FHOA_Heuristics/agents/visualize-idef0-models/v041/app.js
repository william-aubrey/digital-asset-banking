// === IDEF0 Diagrammer Script (v0.4.1) ===
(function(){
  ...
  function roundedPathFromPoints(pts, r){
    ...
    const rr = Math.max(0, Math.min(r, 60));
    if (rr === 0) {
    ...
    (data.edges || []).forEach(e => {
      if (e.type === 'stub') {
        ...
        if (h) {
          ...
          path.setAttribute('id', e.id || ('edge-' + Math.random().toString(36).slice(2,9)));
