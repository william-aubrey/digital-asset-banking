...
GRANT SELECT ON FUTURE TABLES IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_ANALYST;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_ANALYST;

-- REGULATOR privileges
GRANT USAGE ON WAREHOUSE DAB_WAREHOUSE TO ROLE DAB_REGULATOR;
GRANT USAGE ON DATABASE DAB_DATABASE TO ROLE DAB_REGULATOR;
GRANT USAGE ON SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_REGULATOR;
GRANT SELECT ON ALL TABLES IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_REGULATOR;
GRANT SELECT ON ALL VIEWS IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_REGULATOR;
GRANT SELECT ON FUTURE TABLES IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_REGULATOR;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_REGULATOR;

-- Step 9: Transfer ownership of all objects to the DAB_ADMIN role for proper management
-- Using REVOKE CURRENT GRANTS to handle dependent grants automatically
GRANT OWNERSHIP ON WAREHOUSE DAB_WAREHOUSE TO ROLE DAB_ADMIN REVOKE CURRENT GRANTS;
GRANT OWNERSHIP ON DATABASE DAB_DATABASE TO ROLE DAB_ADMIN REVOKE CURRENT GRANTS;
GRANT OWNERSHIP ON SCHEMA DAB_DATABASE.DAB_SCHEMA TO ROLE DAB_ADMIN REVOKE CURRENT GRANTS;
